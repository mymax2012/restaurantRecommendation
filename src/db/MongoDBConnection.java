...
public class MongoDBConnection implements DBConnection {
	...
	public Set<String> getVisitedRestaurants(String userId) {
		/*
		 * Set<String> set = new HashSet<>(); FindIterable<Document> iterable =
		 * db.getCollection("users").find(new Document("user_id", userId));
		 * 
		 * 
		 * if (iterable.first().containsKey("visited")) { List<String> list =
		 * (List<String>) iterable.first().get("visited"); set.addAll(list); }
		 * 
		 * iterable.forEach(new Block<Document>() {
		 * 
		 * @Override public void apply(final Document document) { if
		 * (document.containsKey(VISITED_TABLE)) { List<String> list =
		 * (List<String>)document.get(VISITED_TABLE); set.addAll(list); } } });
		 * return set;
		 */
	...
	public Set<String> getCategories(String businessId) {
		...
		iterable.forEach(new Block<Document>() {
			...
			public void apply(final Document document) {
				if (document.containsKey("catagories")) {
					String categories = (String) document.get("categories");
